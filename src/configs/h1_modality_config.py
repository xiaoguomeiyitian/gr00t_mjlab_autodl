"""
H1 人形机器人 ModalityConfig — 用于 Isaac-GR00T 微调训练。

在 AutoDL 云端运行：
    python gr00t/experiment/launch_finetune.py \
        --modality-config-path /root/gr00t_mjlab_autodl/h1_modality_config.py \
        ...
...
def get_modality_config() -> dict:
    """返回 h1 的 ModalityConfig dict（不注册）。"""
    return {
    # 视频：当前帧（H1 有两个相机视角 front + wrist）
...
h1_config = get_modality_config()
register_modality_config(h1_config, embodiment_tag=EmbodimentTag.NEW_EMBODIMENT)
