#!/usr/bin/env bash
# ============================================================================
# 第 5 步 (可选): 本地 INT4 量化 — 将 BF16 完整模型转换为 INT4 量化版
#
# 运行环境: 本地 (8GB+ 显存即可)
# 作用: 纯后训练量化 (PTQ), 不需要重新训练, 5-15 分钟完成
#       将 BF16 完整模型 (~7GB) → INT4 量化 (~1.5GB), 适合低显存 GPU 推理
#
# 前置条件:
#   - 已通过 ./scripts/04_download_model.sh 下载 BF16 完整模型
#   - 已安装: pip install transformers peft bitsandbytes accelerate
#
# 用法:
#   # 默认: 从 models/{robot}_gr00t/ → models/{robot}_gr00t_int4/
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -h|--help)
            ...
            echo ""
            echo "说明:"
            echo "  --input  默认: models/\${ROBOT}_gr00t  (fallback: \${ROBOT}_gr00t_full_fp16)"
...
# ── 默认路径 ──────────────────────────────────────────────────────────
# 优先读新名 (03_autodl_train.sh 输出, BF16 完整模型)
# fallback 到旧名 (遗留的 _full_fp16 目录)
if [ -z "$INPUT_DIR" ]; then
    ...
    [ -z "$INPUT_DIR" ] && INPUT_DIR="$PROJECT_ROOT/models/${ROBOT}_gr00t"
fi
...
step "检查输入模型..."
if [ ! -d "$INPUT_DIR" ]; then
    fail "BF16 模型目录不存在: $INPUT_DIR
请先运行: ./scripts/04_download_model.sh root@host -p PORT --robot $ROBOT"
fi
# 检查必要文件
...
echo -e "${GREEN}╚══════════════════════════════════════════════════════════════════╝${NC}"
echo ""
info "BF16 完整: $INPUT_DIR  ($INPUT_SIZE)"
